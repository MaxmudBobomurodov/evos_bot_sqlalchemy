...
async def get_category_id(message: types.Message, state: FSMContext, session):
    ...
    if product_id:
        await message.answer(f"✅ Product added with ID: {product_id}")
    else:
        await message.answer("❌ Error adding product.")
